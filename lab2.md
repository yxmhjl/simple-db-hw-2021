...
要删除一个元组，你需要实现‘ deleteTuple ’。元组包含‘ RecordIDs ’，它允许您找到它们所在的页面，因此这应该像定位元组所属的页面并适当修改页面标题一样简单。
...
‘ HeapFile.java ’中的‘ insertTuple ’方法负责将元组添加到堆文件中。要向HeapFile添加一个新的元组，您必须找到一个具有空槽的页面。如果HeapFile中不存在这样的页面，则需要创建一个新页面并将其附加到磁盘上的物理文件。您需要确保元组中的RecordID被正确更新
